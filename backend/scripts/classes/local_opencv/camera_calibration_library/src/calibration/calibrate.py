...
class CharucoCalibrator:
    def __init__(self, aruco_dict: int, squares_vertically: int, squares_horizontally: int, square_length: float, marker_length:float, calibration_images_dir:str = None, raw_images_dir:str = None):
        ...
        Args:
            ...
            calibration_images_dir (str, optional): Directory containing calibration data.
            raw_images_dir (str, optional): Directory containing raw data to be processed.
        """
    ...
    def image_generator(self, folder_path) -> Generator[Tuple[str, np.ndarray], None, None]:
        ...
        Args:
            folder_path (str): The path to the directory containing data.
    ...
    def calibration_images(self) -> Generator[Tuple[str, np.ndarray], None, None]:
        """
        Generator function that yields data from the calibration data directory.
    ...
    def raw_images(self) -> Generator[Tuple[str, np.ndarray], None, None]:
        """
        Generator function that yields data from the raw data directory.
    ...
    def detect_charuco_corners(
        ...
        """
        Detects ChArUco corners in an image. Forces corners if interpolation fails and reshapes for fisheye calibration.
        ...
        Returns:
            Optional[Tuple[np.ndarray, np.ndarray]]: A tuple of (charuco_ids, charuco_corners) if corners are detected, otherwise None.
                                                     charuco_corners is shaped (N,1,2) and dtype float32 for fisheye calibrate.
        """
        ...
        if marker_ids is not None and len(marker_ids) > 0:
            ...
            # Step 3: Force corners if interpolation fails
            if not charuco_retval or charuco_corners is None or len(charuco_corners) == 0:
                if verbose:
                    print(f"Interpolation failed in {image_name}, forcing ChArUco corners using marker centers.")
                charuco_corners = []
                charuco_ids = []

                for i, c in enumerate(marker_corners):
                    # Take center of marker
                    center = c[0].mean(axis=0)
                    charuco_corners.append(center)
                    # Assign sequential ChArUco corner IDs (0 ... N-1)
                    charuco_ids.append(i)

                charuco_corners = np.array(charuco_corners, dtype=np.float32)
                charuco_ids = np.array(charuco_ids, dtype=np.int32)

            # Step 4: Reshape for fisheye calibration (N,1,2)
            charuco_corners = charuco_corners.reshape(-1, 1, 2)

            if verbose:
                print(f"Detected {len(charuco_corners)} charuco corners in {image_name}")
...
class FisheyeCalibrator(CharucoCalibrator):
    ...
    def calibrate(self, grayscale = True, calibration_filename ='fisheye_calibration.json', window_size=(480,480),verbose = True) -> None:
        ...
        Args:
            grayscale (bool): Whether to convert data to grayscale. Defaults to True.
        ...
        square_len = self.board.getSquareLength()  # in meters or consistent units

        for corners, ids in zip(all_charuco_corners, all_charuco_ids):
            if len(corners) >= 4:
                count += 1

                # Generate object points for this image using sequential ChArUco IDs
                obj_points = []
                for cid in ids:
                    row = cid // squares_x
                    col = cid % squares_x
                    obj_points.append([col * square_len, row * square_len, 0.0])
        ...
        print('Number of valid images used for calibration:', count)
    ...
    def export_camera_params_colmap(self, calibration_path: str = None) -> None:
        ...
        if calibration_path is None:
            calibration_path = os.path.join(
                os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
                'scripts', 'data', 'calibration', 'camera_intrinsics', 'fisheye_calibration.json'
            )
